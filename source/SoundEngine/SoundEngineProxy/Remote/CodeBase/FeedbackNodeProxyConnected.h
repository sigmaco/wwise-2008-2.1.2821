...
class FeedbackNodeProxyConnected
	: public ParameterNodeProxyConnected
{
public:
	FeedbackNodeProxyConnected( AkUniqueID in_id );
	virtual ~FeedbackNodeProxyConnected();
	virtual void HandleExecute( CommandDataSerializer& in_rSerializer, CommandDataSerializer& out_rReturnSerializer );
	virtual AudioNodeProxyLocal& GetLocalProxy() { return m_proxy; }

protected:
	FeedbackNodeProxyLocal m_proxy;

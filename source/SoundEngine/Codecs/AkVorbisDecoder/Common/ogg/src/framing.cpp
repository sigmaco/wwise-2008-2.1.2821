...
static ogg_buffer_state *ogg_buffer_create(void){
  ogg_buffer_state *bs=(ogg_buffer_state*)_ogg_calloc(1,sizeof(*bs));
  return bs;
}
...
static ogg_buffer *_fetch_buffer(ogg_buffer_state *bs,long bytes){
  ...
  if(bs->unused_buffers){
    ...
    if(ob->size<bytes){
      ob->data=(unsigned char*)_ogg_realloc(ob->data,bytes);
      ob->size=bytes;
    }
  }else{
    /* allocate a new buffer */
    ob=(ogg_buffer*)_ogg_malloc(sizeof(*ob));
    ob->data=(unsigned char*)_ogg_malloc(bytes<16?16:bytes);
    ob->size=bytes;
  }
...
static ogg_reference *_fetch_ref(ogg_buffer_state *bs){
  ...
  if(bs->unused_references){
    ogref=bs->unused_references;
//    bs->unused_references=ogref->next;
    bs->unused_references = NULL;
  }else{
    /* allocate a new reference */
    ogref=(ogg_reference*)_ogg_malloc(sizeof(*ogref));
  }
...
//  ogg_buffer    *ob=_fetch_buffer(bs,bytes);
  ogg_reference *ogref=_fetch_ref(bs);
//  ogref->buffer=ob;
  return ogref;
}

/* enlarge the data buffer in the current link */
static void ogg_buffer_realloc(ogg_reference *ogref,long bytes){
//  ogg_buffer    *ob=ogref->buffer;
	ogg_buffer* ob = &(ogref->buffer);
...
static void _ogg_buffer_mark_one(ogg_reference *ogref){
//  ogref->buffer->refcount++;
	ogref->buffer.refcount++;
}

/* increase the refcount of the buffers to which the reference points */
static void ogg_buffer_mark(ogg_reference *ogref){
  while(ogref){
    _ogg_buffer_mark_one(ogref);
//    ogref=ogref->next;
    ogref = NULL;
  }
}
...
static ogg_reference *ogg_buffer_sub(ogg_reference *ogref,long length){
  ogg_reference *ret=0,*head=0;
  
  /* duplicate the reference chain; increment refcounts */
  while(ogref && length){
//    ogg_reference *temp=_fetch_ref(ogref->buffer->ptr.owner);
	ogg_reference* temp = _fetch_ref(ogref->buffer.ptr.owner);
//    if(head)
//      head->next=temp;
//    else
      ret=temp;
    head=temp;
    head->buffer=ogref->buffer;    
    head->begin=ogref->begin;
    head->length=length;
    if(head->length>ogref->length)
      head->length=ogref->length;
...
ogg_reference *ogg_buffer_dup(ogg_reference *ogref){
  ogg_reference *ret=0,*head=0;
  /* duplicate the reference chain; increment refcounts */
  while(ogref){
//    ogg_reference *temp=_fetch_ref(ogref->buffer->ptr.owner);
...
static ogg_reference *ogg_buffer_split(ogg_reference **tail,
  ...
  ogg_reference *ogref=*tail;

  while(ogref && pos>ogref->length){
    pos-=ogref->length;
//    ogref=ogref->next;
    ogref = NULL;
  }
...
//      }
      ...
      long lengthA=pos;
      long beginB=ogref->begin+pos;
      long lengthB=ogref->length-pos;
      
      /* make a new reference to tail the second piece */
//      *tail=_fetch_ref(ogref->buffer->ptr.owner);
	*tail = _fetch_ref(ogref->buffer.ptr.owner);
      
      (*tail)->buffer=ogref->buffer;
      (*tail)->begin=beginB;
      (*tail)->length=lengthB;
//      (*tail)->next=ogref->next;
      _ogg_buffer_mark_one(*tail);
      if(head && ogref==*head)*head=*tail;    
      
      /* update the first piece */
//      ogref->next=0;
      ogref->length=lengthA;
...
static void ogg_buffer_release_one(ogg_reference *ogref){
//  ogg_buffer *ob=ogref->buffer;
	ogg_buffer* ob = &(ogref->buffer);
...
//  ogref->next=bs->unused_references;
  bs->unused_references=ogref;
...
static void ogg_buffer_release(ogg_reference *ogref){
  while(ogref){
//    ogg_reference *next=ogref->next;
    ogg_reference *next = NULL;
    ogg_buffer_release_one(ogref);
    ogref=next;
  }
}

static ogg_reference *ogg_buffer_pretruncate(ogg_reference *ogref,long pos){
...
static ogg_reference *ogg_buffer_walk(ogg_reference *ogref){
  if(!ogref)return NULL;
//  while(ogref->next){
//    ogref=ogref->next;
//  }
  return(ogref);
...
static int oggbyte_init(oggbyte_buffer *b,ogg_reference *ogref){
  memset(b,0,sizeof(*b));
  if(ogref){
    b->ref=b->baseref=ogref;
    b->pos=0;
    b->end=b->ref->length;
//    b->ptr=b->ref->buffer->data+b->ref->begin;  
	b->ptr = b->ref->buffer.data + b->ref->begin;  
...
ogg_sync_state *ogg_sync_create(void){
  ogg_sync_state *oy=(ogg_sync_state*)_ogg_calloc(1,sizeof(*oy));
...
//    return oy->fifo_head->buffer->data+oy->fifo_head->begin;
  ...
  {
    ogg_reference* newogref=(ogg_reference*)ogg_buffer_alloc(oy->bufferpool,bytes);
//    oy->fifo_head->next=newogref;
    oy->fifo_head=newogref;
  }
//  return oy->fifo_head->buffer->data;
	return oy->fifo_head->buffer.data;
}
...
static ogg_uint32_t _checksum(ogg_reference *ogref, int bytes){
  ogg_uint32_t crc_reg=0;
  int j,post;

  while(ogref){
//    unsigned char *data=ogref->buffer->data+ogref->begin;
	unsigned char* data = ogref->buffer.data+ogref->begin;
    post=(bytes<ogref->length?bytes:ogref->length);
    for(j=0;j<post;++j)
...
ogg_stream_state *ogg_stream_create(int serialno){
  ogg_stream_state *os=(ogg_stream_state*)_ogg_calloc(1,sizeof(*os));
...
long ogg_sync_pageseek(ogg_sync_state *oy,ogg_page *og){
 ...
 sync_fail:
  ...
  while(oy->fifo_tail){
    ...
    unsigned char *next=(unsigned char*)memchr(now, 'O', oy->fifo_tail->length);

...
int res_unpack(vorbis_info_residue *info,vorbis_info *vi,oggpack_buffer *opb, CAkVorbisAllocator& VorbisAllocator)
{
	int j,k,AllocSize;
	...
	AllocSize = info->partitions * sizeof(*info->stagemasks) + info->partitions * 8 * sizeof(*info->stagebooks);
	vi->AddAllocatedMemory(AllocSize,"info->stagemb");
...
{
	int i,j,k,s,used=0;
	codec_setup_info* ci = (codec_setup_info *)&vd->vi.csi;
...
#ifdef _CHECKSTACKALLOC
				int Size = ch*sizeof(*partword);
#endif
				for(j=0;j<ch;j++)
				{
					partword[j]=(char *)alloca(partwords*partitions_per_word*sizeof(*partword[j]));
#ifdef _CHECKSTACKALLOC
					Size += partwords*partitions_per_word*sizeof(*partword[j]);
#endif
				}
				...
				for(s=0;s<info->stages;s++)
				{
					for(i=0;i<partvals;)
					{
						if(s==0)
						{
							...
							for(j=0;j<ch;j++)
							{
								int temp=vorbis_book_decode(phrasebook,GET_OGGPACK_BUFFER());
						...
						for(k=0;k<partitions_per_word && i<partvals;k++,i++)
						{
							for(j=0;j<ch;j++)
							{
								...
								if(info->stagemasks[(int)partword[j][i]]&(1<<s))
								{
									...
									if(info->type)
									{
										if(vorbis_book_decodev_add(stagebook,in[j]+offset,GET_OGGPACK_BUFFER(),samples_per_partition,-8)==-1)
										{
											goto eopbreak;
										}
									}
									...
									{
										if(vorbis_book_decodevs_add(stagebook,in[j]+offset,GET_OGGPACK_BUFFER(),samples_per_partition,-8)==-1)
										{
											goto eopbreak;
										}
									}
...
#ifdef _CHECKSTACKALLOC
			Size = partwords*partitions_per_word*sizeof(*partword);
			Stack.Alloc(Size);
#endif
			...
			for(s=0;s<info->stages;s++)
			{
				for(i=0;i<partvals;)
				{
					if(s==0)
					{
						...
						temp=vorbis_book_decode(phrasebook,GET_OGGPACK_BUFFER());
						if(oggpack_eop(GET_OGGPACK_BUFFER()))
						{
							goto eopbreak;
						}
					...
					for(k=0;k<partitions_per_word && i<partvals;k++,i++)
					{
						if(info->stagemasks[(int)partword[i]]&(1<<s))
						{
							...
							if(vorbis_book_decodevv_add(stagebook,in,i*samples_per_partition+beginoff,ch,GET_OGGPACK_BUFFER(),samples_per_partition,-8)==-1)
							{
								goto eopbreak;
							}
						}

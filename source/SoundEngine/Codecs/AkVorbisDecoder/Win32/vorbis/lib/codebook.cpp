								...
								CAkVorbisAllocator&	VorbisAllocator)
{
	...
	if(s->dec_nodeb==4)
	{
		...
		Mem += AllocSize;
#if defined(_TRACKMEMORY)
		printf("s->dec_table,%d\n",AllocSize);
#endif
		/* +1 (rather than -2) is to accommodate 0 and 1 sized books,
		which are specialcased to nodeb==4 */
		if(_make_words(lengthlist,s->entries,(ogg_uint32_t*)s->dec_table,quantvals,s,opb,maptype))
		{
			return -1;
		}
		return Mem;
...
int vorbis_book_unpack(oggpack_buffer *opb,codebook *s, CAkVorbisAllocator& VorbisAllocator)
{
	...
	case 1:
		...
		case 0:
			...
			Mem = _make_decode_table(s,lengthlist,quantvals,opb,maptype,VorbisAllocator);
			if(Mem < 0)
			{
				goto _errout;
			}
		...
		case 1:
			...
			/* dec_type choices here are 1,2; 3 doesn't make sense */
			{
				...
				if(total1<=4 && total1<=total2)
				{
					...
					int Mem = _make_decode_table(s,lengthlist,quantvals,opb,maptype,VorbisAllocator);
					if(Mem < 0)
					{
						s->q_val=0; /* cleanup must not free alloca memory */
						goto _errout;
		...
		case 2:
			...
			else
			{
				...
				Mem = s->q_pack * s->used_entries;
				s->q_val = VorbisAllocator.Alloc(Mem);
				AllocatedMem += Mem;
#if defined(_TRACKMEMORY)
				printf("s->q_val,%d\n",Mem);
#endif
...
{
	int step=n/book->dim;
	const long AllocSize = sizeof(ogg_int32_t)*book->dim;

	ogg_int32_t *v = (ogg_int32_t *)alloca(AllocSize);

#ifdef _CHECKSTACKALLOC
	Stack.Alloc(sizeof(*v)*book->dim);
#endif
	int i,j,o;
								...
								int				point)
{
	...
	for(i=0;i<n;)
	{
		int erreur = decode_map(book,b,v,point);
		if(erreur != 0)
		{
			return -1;
		}
